	...
	getRouter() {
		...
		this.router.get("/get-all-users/:ccc_id/:current_iteration", async (req, res) => {
			try {
				...
				const studentAttrs = [
					"id", "first_name", "middle_name", "last_name", "extension_name", "suffix_name",
		...
		this.router.post("/register-supervisor", async (req, res) => {
			try {
				...
				const supervisor = await User.create({
					...
					suffix_name: suffix_name ? suffix_name.trim() : null,
				...
				return res.json({
					...
					supervisor: {
						...
						suffix_name: supervisor.suffix_name,
						extension_name: supervisor.extension_name,
						ccc_id: supervisor.ccc_id,
		...
		this.router.post("/update-user/:ccc_id", async (req, res) => {
			try {
				...
				await User.update({
					...
					last_name,
					suffix_name: suffix_name ?? student.suffix_name,
